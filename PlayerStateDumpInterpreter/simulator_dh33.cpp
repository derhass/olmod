...
Derhass33::Derhass33(ResultProcessor& rp) :
	Cow1(rp),
	...
	fixedDeltaTime(1.0f/60.0f)
{
	...
	instrp[INSTR_HARD_SYNC].name = "DH33_HARD_SYNC";
	instrp[INSTR_SOFT_SYNC].name = "DH33_SOFT_SYNC";
	instrp[INSTR_INTERPOLATE].name = "DH33_INTERPOLATE";
	instrp[INSTR_INTERPOLATE_01].name = "DH33_INTERPOLATE_01";
	instrp[INSTR_INTERPOLATE_12].name = "DH33_INTERPOLATE_12";
	instrp[INSTR_INTERPOLATE_23].name = "DH33_INTERPOLATE_23";
	instrp[INSTR_EXTRAPOLATE].name = "DH33_EXTRAPOLATE";
	instrp[INSTR_EXTRAPOLATE_PAST].name = "DH33_EXTRAPOLATE_PAST";
}
...
const char *Derhass33::GetBaseName() const
{
	return "derhass33";
}

void Derhass33::EnqueueToRing(const PlayerSnapshotMessage& msg, bool wasOld)
{
...
void Derhass33::ResetForNewMatch()
{
	ClearRing();
...
void Derhass33::AddNewPlayerSnapshot(const PlayerSnapshotMessage& msg, bool wasOld)
{
	...
	float scaledDeltaTime = fixedDeltaTime / gs.timeScale;
	if  (m_last_messages_ring_count == 0) {
		// first packet
		EnqueueToRing(msg, false);
		m_last_update_time = gs.lastRealTimestamp;
	} else {
	...
	float delta = (gs.lastRealTimestamp - m_last_update_time) / scaledDeltaTime; // in ticks
	rpcAux[AUX_BUFFER_UPDATE]->Add(m_last_update_time);
	...
	rpcAux[AUX_BUFFER_UPDATE]->Add(m_last_update_time);
	rpcAux[AUX_BUFFER_UPDATE]->Add(gs.timeScale);
	rpcAux[AUX_BUFFER_UPDATE]->FlushCurrent();
}

void Derhass33::DoBufferEnqueue(const PlayerSnapshotMessage& msg, const EnqueueInfo& enqueueInfo)
{
...
bool Derhass33::DoInterpolation(const InterpolationCycle& interpolationInfo, InterpolationResults& results)
{
	SimulatorBase::DoInterpolation(interpolationInfo, results);
	float now = interpolationInfo.unscaledTimestamp; // needs to be the same time source we use for m_last_update_time
	...
	*/
	...
	delta_t *= interpolationInfo.timeScale;
...
void Derhass33::interpolatePlayer(const PlayerSnapshot& A, const PlayerSnapshot& B, PlayerSnapshot& result, float t)
{
	result.id = A.id;
