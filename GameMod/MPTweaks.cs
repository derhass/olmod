...
namespace GameMod {
    class MPTweaks
    {
        ...
        public static string ApplySetting(string key, string value)
        {
            ...
            if (key == "joystick.rotation.fix" && bool.TryParse(value, out bool valJoystickFix))
            {
                Debug.Log("MPTweaks: received server capability for the scaled up rotation");
                JoystickRotationFix.server_support = valJoystickFix;
                return Boolean.TrueString;
            }
            return null;
        }
    ...
    class MPTweaksLoadScene
    {
        private static void Postfix()
        {
            ...
            tweaks.Add("joystick.rotation.fix", Boolean.TrueString);
            if (tweaks.Any())
            {
    ...
    class MPTweaksAcceptedToLobby
    {
        private static void Prefix(AcceptedToLobbyMessage accept_msg)
        {
            ...
            caps.Add("SupportsTweaks", "sniper,jip,nocompress_0_3_6,joystick.rotation.fix");
