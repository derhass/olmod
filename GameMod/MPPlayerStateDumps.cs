...
namespace GameMod {
	public class MPPlayerStateDump {
		...
		*/
		...
		public static Vector3 previousShipVelocity = new Vector3();
		public static Vector3 previousShipAngularVelocity = new Vector3();
		public static bool playerShipOverridden = false;

		[HarmonyPatch(typeof(GameManager), "FixedUpdate")]
		class MPPlayerStateDump_SmoothHack1 {
			static void Prefix() {
				if (playerShipOverridden) {
					...
					GameManager.m_local_player.c_player_ship.c_rigidbody.velocity = previousShipVelocity;
					GameManager.m_local_player.c_player_ship.c_rigidbody.angularVelocity = previousShipAngularVelocity;
					playerShipOverridden = false;
				}
			}
			static void Postfix() {
				if (!Server.IsActive() && GameplayManager.IsMultiplayerActive && NetworkMatch.InGameplay() && !GameManager.m_local_player.c_player_ship.m_dying) {
					...
					previousShipVelocity = GameManager.m_local_player.c_player_ship.c_rigidbody.velocity;
					previousShipAngularVelocity = GameManager.m_local_player.c_player_ship.c_rigidbody.angularVelocity;
